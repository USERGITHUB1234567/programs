...
int m,k,d[maxn],pairX[maxn],pairY[maxn];
vector<int>bank[26],adj[maxn];
string s;
bool bfs() {
    queue<int>q;
    for(int i=1; i<=k; ++i) {
        if(!pairX[i]) {q.push(i),d[i]=0;}
        else d[i]=-1;
    }
    d[0]=-1;
    while(!q.empty()) {
...
int hopcroft_karp() {
    int match=0;
    while(bfs()) {
        for(int i=1; i<=k; ++i) {if(!pairX[i] && dfs(i)) ++match;}
    }
    return match;
}
int main(int argc, char** argv) { 
    ...
    for(int i=0; i<k; ++i) {
        int t=s[i]-'A';
        for(int j:bank[t]) adj[i+1].pb(j);
    }
    cout << hopcroft_karp() << '\n';
    for(int i=1; i<=k; ++i) {
        if(pairX[i]) cout << i << ' ' << pairX[i] << '\n';
    }
    return 0; 
