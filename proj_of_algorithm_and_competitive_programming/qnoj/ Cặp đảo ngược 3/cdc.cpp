...
bool cmp(pair<int,int>a,pair<int,int>b) {return a.fi<b.fi || (a.fi==b.fi && a.se<b.se);}
int n,a[maxn],cur=0;
long long kq=0,ex=0;
struct fenwick_tree {
    long long bit[maxn];
...
int main() {
    ...
    //file("cdc")
    scanf("%d",&n);
    for(int i=1; i<=n; ++i) {scanf("%d",&a[i]);ans.pb({a[i],i});ft.upd(i,1);}
    sort(all(ans),cmp);
    for(int i=0; i<n; ++i) {
        while(ans[cur].fi<i && cur<n) {
            int j=ans[cur].se;
            kq+=ft.sum(j-1);
            ft.upd(j,-1);
            ++cur;
