...
inline vect vect_add(vect a,vect b) {return {a.x+b.x,a.y+b.y};}
inline vect vect_sub(vect a,vect b) {return {a.x-b.x,a.y-b.y};}
inline vect vect_mul(vect a,int t) {return {t*a.x,t*a.y};}
...
namespace cookedsoup{
    vect pre[maxn],suf[maxn];
    void solve() {
        for(int i=1; i<=n; ++i) {
            auto[x,y]=p[i];
            pre[i]=vect_add(pre[i-1],{x,y});
            auto[u,v]=p[n-i+1];
            suf[n-i+1]=vect_add(suf[n-i+2],{u,v});
        }
        vector<point>m;
        for(int i=1; i<=n; ++i) m.pb(p[i]);
        long double a=area(m),sum=0;
        for(int i=1; i<=n; ++i) {
            auto[x,y]=p[i];
            vect oa={x,y};
            vect t1=vect_sub(vect_mul(oa,i-1),pre[i-1]),t2=vect_sub(suf[i+1],vect_mul(oa,n-i));
            sum+=cross_product(t1,t2);
        }
        sum/=2;
