...
long long ans=0;
vector<int>res;
struct edge{int u,v; long long w; int id;}e[1003];
struct dinitz{
    ...
    void init() {
        ...
        for(int i=1; i<=m; ++i) {
            if(!rem[i]) {
                int u = e[i].u, v = e[i].v;
                long long w = e[i].w * 1005LL + 1LL;
                cap[u][v]+=w;
            }
        }
        for(int i=1; i<=n; ++i) {
            for(int j=1; j<=n; ++j) {
                if(cap[i][j] > 0 || cap[j][i] > 0) adj[i].pb(j);
            }
        }
        s=1,t=n;
    }
    void bfs() {
        ...
        while(!q.empty()) {
            int u=q.front();q.pop();
            for(int v:adj[u]) {
                if(d[v]==-1 && cap[u][v]-flow[u][v]>0) {q.push(v);d[v]=d[u]+1;}
            }
        }
    }
    long long dfs(int u, long long f) {
        ...
        for(;ptr[u]<adj[u].size(); ++ptr[u]) {
            ...
            long long push=dfs(v,min(f,cap[u][v]-flow[u][v]));
            if(push) {
                flow[u][v]+=push;
                flow[v][u]-=push;
                return push;
    ...
    long long maxflow() {
        ...
        while(true) {
            bfs();
            if(d[t]==-1) break;
            while(long long push=dfs(s,3e18)) total+=push;
        }
        return total;
    }
};
...
int main(int argc, char** argv) { 
    ...
    long long cur = maxflow;
    for(int i=1; i<=m; ++i) {
        rem[i]=true;
        dinitz tmp(n);
        long long w = e[i].w * 1005LL + 1LL;
        long long t = tmp.maxflow();
        if(t == cur - w) {
            cur = t;
            res.pb(e[i].id);
        }
    ...
    long long min_cost = maxflow / 1005LL;
    long long min_edges = maxflow % 1005LL;
    cout << min_cost << ' ' << min_edges << '\n';
