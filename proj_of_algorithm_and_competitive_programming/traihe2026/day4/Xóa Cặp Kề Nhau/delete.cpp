...
int n,q,c[maxn],st[maxn],val[maxn],p[maxn],up[maxn][20],logn,d[maxn];
...
int main(int argc, char** argv) { 
    ...
    cin >> n >> q;for(int i=1; i<=n; ++i) cin >> c[i];
    st[0]=1;
    int nver=1,curv=1;
    for(int i=1; i<=n; ++i) {
        if(c[i]==val[curv] && curv!=1) curv=p[curv];
        else {
            auto it=adj[curv].find(c[i]);
            if(it==adj[curv].end()) {
                ++nver;
                adj[curv][c[i]]=nver;
                p[nver]=curv;
    ...
    for(int i=1; i<=nver; ++i) up[i][0]=p[i];
    logn=31-__builtin_clz(nver);
    for(int j=1; j<=logn; ++j) {
        for(int i=1; i<=nver; ++i) up[i][j]=up[up[i][j-1]][j-1];
    }
