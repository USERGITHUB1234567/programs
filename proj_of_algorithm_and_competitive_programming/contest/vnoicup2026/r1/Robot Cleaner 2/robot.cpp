...
inline int solve(int &n, int &m,int &r,int &c) {
    int cntr=0,cntc=0,cnt=0,dr=1,dc=1;
    fill(row+1,row+1+n,false);
    fill(col+1,col+1+m,false);
    row[r]=true;
    col[c]=true;
    ++cntr,++cntc;
    while(cntr<n && cntc<m) {
        ...
        if(c==1 && dc==-1) dc=1;
        r+=dr,c+=dc;
        if(!row[r]) {++cntr;row[r]=true;}
        if(!col[c]) {++cntc;col[c]=true;}
