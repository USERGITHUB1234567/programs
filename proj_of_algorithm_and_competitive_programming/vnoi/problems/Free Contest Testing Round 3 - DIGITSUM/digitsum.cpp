...
long long a,b,f[2][20][200];
int x[20];
inline long long process(int id, bool smaller, int sum) {
    if(id<0) return sum;
    long long &mem=f[smaller][id][sum];
    if(mem!=-1) return mem;
    int lim=(smaller?9:x[id]);
    mem=0;
    for(int digit=0; digit<=lim; ++digit) mem+=process(id-1,smaller || (digit<lim),sum+digit);
    return mem;
}
