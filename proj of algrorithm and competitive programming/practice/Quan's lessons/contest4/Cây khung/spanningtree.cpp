...
class disjoint_set_union{
    private:
        vector<int> p;
    public:
        disjoint_set_union(int n=0) { if(n>0) init(n); }
        void init(int n) { p.assign(n+1,0); for(int i=1;i<=n;++i) p[i]=i; }
...
int n,m,ncomp,up[maxn][20],mn[maxn][20],d[maxn];
int main(int argc, char** argv) {
    ...
    vector<int> cur; cur.reserve(max(0, n-1));
    for(int i=1; i<=m; ++i) {
        cin >> ed[i].u >> ed[i].v >> ed[i].w;
        if(dsu.check(ed[i].u,ed[i].v)) {
            vector<int> cand = cur;
            cand.push_back(i);
            sort(cand.begin(), cand.end(), [&](int a, int b){ return ed[a].w < ed[b].w; });
            dsu.init(n);
            cur.clear();
            ncomp = n;
            for(int ei : cand) {
                if(!dsu.check(ed[ei].u, ed[ei].v)) {
                    dsu.unite(ed[ei].u, ed[ei].v);
                    cur.push_back(ei);
                    --ncomp;
                }
            }
        ...
        if(ncomp>1) { cout << uncon << '\n'; continue; }
        long long ans=0;
        if(!cur.empty()) for(int ei:cur) ans += ed[ei].w;
        cout << ans << '\n';
    }
    return 0;
}
