...
class segment_tree{
    private:
        struct tree{long long sum,mx,l,r;}st[maxn<<2];
    public:
        long long x[maxn];
        tree mer(const tree& a, const tree& b) {
            ...
            ans.mx=max({a.mx,b.mx,a.r+b.l});
            return ans;
        }
        void build(int id, int l, int r) {
            if(l==r) {
                st[id]={x[l],max(0ll,x[l]),x[l],x[l]};
                return;
            }
            int mid=(l+r)>>1;
            build(id<<1,l,mid);
        ...
        tree query(int id, int l, int r, int i, int j) {
...
int main(int argc, char** argv) { 
    ...
    cin >> n >> q;
    for(int i=1; i<=n; ++i) cin >> seg.x[i];
    seg.build(1,1,n);
    while(q--) {
        int a,b;cin >> a >> b;
        cout << seg.query(1,1,n,a,b).mx << '\n';
    }
    return 0; 
