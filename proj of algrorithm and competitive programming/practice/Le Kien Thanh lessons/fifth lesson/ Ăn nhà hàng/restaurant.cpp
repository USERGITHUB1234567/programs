...
public:
    closure_problem(int _n):n(_n), maxflow(0) {
        s = 0;
        t = n + 1;
        adj.resize(n + 2);
        level.resize(n + 2);
    ...
    inline void add_edge(int u, int v, long long cap) {
        adj[u].pb({v, cap, 0, (int)adj[v].size()});
        adj[v].pb({u, 0, 0, (int)adj[u].size() - 1});
    }
    
    inline void set_weight(int u, long long w) {
        if (w > 0) add_edge(s, u, w);
        else if (w < 0) add_edge(u, t, -w);
    }
    
    inline void set_relationship(int u, int v) {
        add_edge(u, v, inf);
    }
    ...
    long long dfs(int u, long long pushed) {
        ...
        for(int& cid = ptr[u]; cid < (int)adj[u].size(); ++cid) {
            ...
            long long push = dfs(v, min(pushed, edge.cap - edge.flow));
            if(push == 0) continue;
            edge.flow += push;
...
int main(int argc, char** argv) { 
    ...
    int max_nodes = n * n + n;
    closure_problem graph(max_nodes);
    
    for(int i=1; i<=n; ++i) graph.set_weight(i, -c[i]);
    
    for(int i=1; i<=n; ++i) {
        for(int j=1; j<=n; ++j) {
            int cur = n + (i - 1) * n + j;
            long long w = 1LL * a[i] * b[j];
            ans += w;
            graph.set_weight(cur, w);
            graph.set_relationship(cur, i);
            if(i != j) graph.set_relationship(cur, j);
        }
    }
    
    for(int i=1, u, v; i<=m; ++i) {
        cin >> u >> v;
        graph.set_relationship(u, v);
    }
