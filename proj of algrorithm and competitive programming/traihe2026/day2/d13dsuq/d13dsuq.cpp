...
const int maxn=10000001,mod=1000000007,maxb=320; 
namespace utilities{ 
...
int main(int argc, char** argv) { 
    ...
    cin >> n >> q >> x >> y >> z >> m;
    a[1]=x;
    for(int i=2; i<=n; ++i) a[i]=(1ll*a[i-1]*y+z)%m;
    seg.build(1,1,n);
    long long ans=0;
    for(long long i=1; i<=q; ++i) {
        int l=min(i%n+1,(i*i)%n+1),r=max(i%n+1,(i*i)%n+1);
        ans=(ans+seg.query(1,1,n,l,r))%m;
    }
    cout << ans;
    return 0; 
