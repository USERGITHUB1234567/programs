...
int n;
inline int ask(string s, int i, int j) {
    int res;
    cout << s << ' ' << i << ' ' << j << endl;
    cin >> res;
    return res;
...
int main(int argc, char** argv) { 
    ...
    cin >> n;
    int xor12=ask("XOR",1,2),xor13=ask("XOR",1,3),xor23=xor12^xor13;
    int x=xor12+2*ask("AND",1,2),y=xor13+2*ask("AND",1,3),z=xor23+2*ask("AND",2,3);
    vector<int>res(n+1);
    res[1]=(x+y-z)>>1;
    res[2]=x-res[1];
    res[3]=y-res[1];
