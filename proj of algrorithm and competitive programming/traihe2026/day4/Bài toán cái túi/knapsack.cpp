...
const int maxn=502,mod=1000000007,maxb=320; 
namespace utilities{ 
...
int n,s,w[maxn],v[maxn];
namespace classical_knapsack{
    ll f[300005];
    ll solve(int _s) {
        if(_s <= 0) return 0;
        for(int i=1; i<=_s; ++i) {
...
namespace soupfull{
    ...
    void solve() {
        int best=1;
        for(int i=2; i<=n; ++i) {
            long long t1=1ll*v[best]*w[i],t2=1ll*v[i]*w[best];
            if(t1<t2) best=i;
            else if(t1==t2 && w[i]<w[best]) best=i;
        }
        int quantity=0;
        while(s-(quantity+1)*w[best]>=250000) ++quantity;
        long long base=1ll*v[best]*quantity,extra=0;
        for(int i=0; i<=s-w[best]*quantity; ++i) {
            for(int j=1; j<=n; ++j) {
                if(i>=w[j]) f[i]=max(f[i],f[i-w[j]]+v[j]);
                extra=max(extra,f[i]);
            }
...
int main(int argc, char** argv) { 
    ...
    cin >> n >> s;for(int i=1; i<=n; ++i) cin >> w[i];for(int i=1; i<=n; ++i) cin >> v[i];
    //classical_knapsack::solve();
    soupfull::solve();
    return 0; 
} 
/**/