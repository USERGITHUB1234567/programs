...
int main(int argc, char** argv) { 
    ...
    // Sửa lỗi 3: Kiểm tra pivot != 1 trước khi hỏi
    if(pivot != 1 && ask(1,pivot)==pivot) {
        r=pivot-1;
        while(l<r) {
            int mid=(l+r+1)>>1;
            if(ask(mid,pivot)==pivot) l=mid;
            else r=mid-1;
        }
        cout << "! " << l << endl;
    }
        ...
        while(l<r) {
            int mid=(l+r)>>1;
            // Sửa lỗi 1 & 2: truy vấn đúng chiều l, r và có phép so sánh
            if(ask(pivot,mid)==pivot) r=mid;
            else l=mid+1;
        }
        cout << "! " << l << endl;
    }
