...
const int maxn=100005,mod=1000000007,maxb=320; // Đã tăng giới hạn maxn lên 100005 để pass bộ test
...
class disjoint_sets_union{
    private:
        ...
        void merge(int id, int u, int v) {
            ...
            rollback.push({v,sz[v]}); rollback.push({u,sz[u]});
            total[id]+=2;
            sz[u]+=sz[v];
            p[v]=u;
            --cur_comp; 
    ...
    public:
        ...
        void get(int id, int l, int r) {
            ...
            while(total[id]>0) {
                update up1 = rollback.top(); rollback.pop();
                update up2 = rollback.top(); rollback.pop();
                p[up1.node]=up1.node; sz[up1.node]=up1.old_sz;
                p[up2.node]=up2.node; sz[up2.node]=up2.old_sz;
                total[id]-=2;
                ++cur_comp;
            }
        }
...
int main(int argc, char** argv) {
    ...
    for(auto &x:ump) {
        ...
        int l=x.second;
        if(l<=k) store.pb({l,k,u,v});
    }
    
    for(auto &it:store) {
        if(it.l<=it.r) dsu.upd(1,0,k,it.l,it.r,{it.u,it.v});
    }
