...
int n,m;
struct query{int l,r;string s;}qu[maxn];
class disjoint_sets_union{
    private:
        int p[maxn],dist[maxn];
    public:
        disjoint_sets_union() {for(int i=0; i<maxn; ++i) {p[i]=i,dist[i]=0;}}
        ...
        inline bool unite(int u, int v, bool x) {
            int ru=root(u),rv=root(v);
            if(ru!=rv) {
                p[rv]=ru;
                dist[rv]=dist[u]^dist[v]^x;
                return true;
            }else return (dist[u]^dist[v])==x;
        }
}dsu;
int main(int argc, char** argv) { 
    ...
    for(int i=1; i<=m; ++i) {
        ...
        val.pb(qu[i].l-1),val.pb(qu[i].r);
    }
    sortunique(val);
    auto get=[&](int x) {return lower_bound(all(val),x)-val.begin();};
    int ans=m;
    for(int i=1; i<=m; ++i) {
        int u=get(qu[i].l-1),v=get(qu[i].r),x=(qu[i].s=="odd"?1:0);
        //cerr << i << ' ' << u << ' ' << v << '\n';
        if(!dsu.unite(u,v,x)) {ans=i-1;break;}
    }
    cout << ans;
    return 0; 
