...
int n,k,s[maxn],d[maxn];
inline long double p(long double t, int i) {return (long double)s[i]*t+d[i];}
inline long double f(long double t) {
    long double mn=LLONG_MAX,mx=LLONG_MIN;
    for(int i=1; i<=n; ++i) mx=max(mx,p(t,i)),mn=min(mn,p(t,i));
    return mx-mn;
}
...
int main() {
    ...
    cout << fixed << setprecision(6) << ternary_search(0,k);
    //get_execution_time();
    return 0;
}