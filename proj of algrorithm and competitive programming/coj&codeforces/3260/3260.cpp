...
namespace cookedsoup {
    long long com[maxn][maxn],f[maxn][maxn],c[maxn];
    inline long long c2n(long long x) {return (x*(x-1))/2;}
    void solve() {
        for(int i=0; i<maxn; ++i) com[i][0]=com[i][i]=1;
        ...
        for(int i=1; i<=cur; ++i) {
            for(int j=1; j<=k; ++j) {
                //if(i==1 && j==1) continue;
                for(int t=0; c2n(t)<=j && t<=c[i]; ++t) {
                    int p=j-c2n(t);
                    //cerr << i-1 << ' ' << p << '\n';
                    //cerr << i-1 << ' ' << p << ' ' << t << ' ' << f[i-1][p] << '\n';
                    f[i][j]=(f[i][j]+(f[i-1][p]*(com[c[i]][t]))%mod)%mod;//cout << f[i][j] << '\n';
                    //cout << i << ' ' << j << ' ' << f[i-1][p] << ' ' << t << ' ' << c[i] << '\n';
                }
            }
        }
        cout << f[cur][k]%mod;
