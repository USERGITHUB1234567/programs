...
struct edmond_karp{
    vector<int>adj[maxn];
    int cap[maxn][maxn],flow[maxn][maxn],trace[maxn],total=0,n,s,t;
    inline void bfs() {
        ...
        while(!q.empty()) {
            int u=q.front();q.pop();
            for(int v:adj[u]) {
                if(trace[v] || flow[u][v]==cap[u][v]) continue;
                trace[v]=u;
                q.push(v);
            }
        }
    ...
    inline void increase() {
        ...
        for(int i=t; i!=s; i=trace[i]) {
            delta=min(delta,cap[trace[i]][i]-flow[trace[i]][i]);
        }
        total+=delta;
        for(int i=t; i!=s; i=trace[i]) {
            flow[trace[i]][i]+=delta;
            flow[i][trace[i]]-=delta;
        }
    }
    inline int maxflow() {
        while(true) {
