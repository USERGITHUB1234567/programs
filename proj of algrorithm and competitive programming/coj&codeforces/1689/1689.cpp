...
int c,d,v,dist[maxn],pairX[maxn],pairY[maxn],n;
vector<int>adj[maxn];
bool bfs() {
    queue<int>q;
    for(int i=1; i<=n; ++i) {
        if(!pairX[i]) {dist[i]=0;q.push(i);}
        else dist[i]=-1;
    }
    dist[0]=-1;
    while(!q.empty()) {
...
int main(int argc, char** argv) { 
    ...
    cin >> c >> d >> v;
    vector<pair<string,string>>dog,cat;
    for(int i=1; i<=v; ++i) {
        string s1,s2;cin >> s1 >> s2;
        if(s1[0]=='D') {dog.pb({s1,s2});}
        else cat.pb({s1,s2});
    }
    n=dog.size();
    for(int i=0; i<dog.size(); ++i) {
        for(int j=0; j<cat.size(); ++j) {
            auto a=dog[i],b=cat[j];
            if(a.fi==b.se || a.se==b.fi) adj[i+1].pb(j+1);
        }
    }
    cout << v-hopcroft_karp();
    return 0; 
