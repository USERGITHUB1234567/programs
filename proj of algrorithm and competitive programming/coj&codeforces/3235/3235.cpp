...
int n,k;
long long c[maxn],sum[320][320],dif[320][maxn];
struct container{
    int a,l,d;
    bool operator<(container o) {return a<o.a;}
};
int main() {
    ...
    sort(all(ops),[](container a, container b){return a<b;});
    int b=sqrt(n);
    int cur=0;
    for(int i=1; i<=n; ++i) {
        for(int d=1; d<=b; ++d) {
            if(dif[d][i]!=0) sum[d][i%d]-=dif[d][i];
            c[i]+=sum[d][i%d];
        }
        while(cur<k && ops[cur].a==i) {
            ...
            else for(int j=0; j<l; ++j) ++c[i+j*d];
            ++cur;
        }
    }
