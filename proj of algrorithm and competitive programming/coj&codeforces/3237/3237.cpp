...
struct disjoint_set_union{
    ...
    bool same(int u, int v) {return found(u)==found(v);}
};
int n,m,q;
...
int main() {
    ...
    for(int i=1; i<=q; ++i) cin >> queries[i].fi >> queries[i].se;
    vector<int>l(q+1,0),r(q+1,m+1);
    while(true) {
        bool stop=true;
        ...
        for(int day=0; day<=m; ++day) {
            if(day!=0) dsu.unite(edge[day].fi,edge[day].se);
            for(int i:bucket[day]) {
                if(dsu.same(queries[i].fi,queries[i].se)) {r[i]=day;}
                else l[i]=day+1;
            }
        }
    }
