...
short m,n,t[maxn][maxn],d[maxn],pairX[maxn],pairY[maxn];
vector<int>adj[maxn];
bool bfs() {
    queue<int>q;
    for(int i=1; i<=n; ++i) {
        if(!pairX[i]) {d[i]=0; q.push(i);} else d[i]=-1;
    }
    d[0]=-1;
    while(!q.empty()) {
        int u=q.front();q.pop();
...
bool check(int x) {
    short match = 0;
    fill(pairX+1, pairX+n+1, 0);
    fill(pairY+1, pairY+m+1, 0);
