...
long long n;
pair<double,double> solve(long long s,long long n) {
    double b=s,c=-n,delta=(b*b)-(4*c);
    if(delta<0) {
...
int main() {
    ...
    scanf("%lld",&n);
    for(int i=0; i<=162; ++i) {
        auto[x1,x2]=solve(i,n);
        if(x1!=LLONG_MAX) {
            if(x1>=0) {
                long long t=(long long)llround(x1);
                if(t>=0 && digit(t)==i) {
                    __int128 lhs=(__int128)t*t+(__int128)i*t;
                    if(lhs==(__int128)n) {
                        printf("%lld",t);
                        return 0;
                    }
            ...
            if(x2 >= 0) {
                long long t=(long long)llround(x2);
                if(t>=0 && digit(t)==i) {
                    __int128 lhs=(__int128)t*t+(__int128)i*t;
                    if(lhs==(__int128)n) {
                        printf("%lld", t);
    ...
    //cout << solve(1,2).fi << ' ' << solve(1,2).se;
    printf("-1");
    return 0;
}