...
int main(int argc, char** argv) { 
    ...
    while(t--) {
        ...
        for(int i=1; i<=n; ++i) {
            if(a[i]==b[i]) {
                if(min(a[i],k-a[i])==cur) continue;
                if(cnta[cur]!=cntb[cur]) {ck=false;break;}
                cnta.clear();
                cntb.clear();
                cur=INT_MAX;
                continue;
            }
            if(b[i]!=k-a[i]) {ck=false;break;}
            //if(i==n) {ck=false;break;}
            //if(a[i+1]!=a[i] || b[i]!=k-a[i]) {ck=false;cerr << i << ' ';break;}
            if(cur!=min(a[i],b[i])) {
                if(cnta[cur]!=cntb[cur]) {ck=false;break;}
