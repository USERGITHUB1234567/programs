...
void add(pair<int,int>q) {
    int l=0,r=ans.size()-1;
    if(ans.empty() || ans.back().fi<q.fi) {
        ans.pb(q);
        return;
    }
    while(l<=r) {
        int m=(l+r)>>1;
        if(ans[m].fi<q.fi) l=m+1;
        else r=m-1;
    }
    if(ans[l].fi==q.fi) ans[l]=q;
    else ans.insert(ans.begin()+l,q);
}
void soupfull() {
    for(int i=1; i<=n; ++i) {
        add(p[i]);
