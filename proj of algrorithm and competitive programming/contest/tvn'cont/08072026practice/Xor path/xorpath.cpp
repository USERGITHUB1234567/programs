...
inline int centroid(int u, int p, int tot) {
    for(int v:adj[u]) {
        if(v!=p && !del[v] && sz[u]>(tot>>1)) return centroid(v,u,tot);
    }
    return u;
}
inline void dfs(int u, int p, int type, int d) {
    //cerr << u << ' ';
    cnt+=d;
    mp[type]+=d;
    ans+=cnt-mp[type];
    for(int v:adj[u]) {
        if(v!=p && !del[v]) dfs(v,u,type,d^a[v]);
    }
}
