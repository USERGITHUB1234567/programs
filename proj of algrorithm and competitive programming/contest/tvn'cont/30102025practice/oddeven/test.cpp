...
inline long long rnd1(long long l, long long r) {
    uniform_int_distribution<int>dist(l,r);
    return dist(generator1);
}
int main() {
    ios::sync_with_stdio(false);cin.tie(nullptr);cout.tie(nullptr);
    freopen("oddeven.inp", "w", stdout);
    int n=rnd1(0,0),m=rnd1(1,5000);
    cout << n;
    for(int i=1; i<=n; ++i) cout << ' ' << rnd1(0,1);
    cout << '\n' << m;
    for(int i=1; i<=m; ++i) cout << ' ' << rnd1(0,1);
    return 0;
}