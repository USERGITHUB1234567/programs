...
long long n,c,t,p[maxn],tim[maxn],pre[maxn],kq=0,cur=0,ans=0;
int main() {
    ios::sync_with_stdio(false);cin.tie(nullptr);cout.tie(nullptr);
    file("chiakeo")
    scanf("%lld%lld%lld",&n,&c,&t);
    for(int i=1; i<=n; ++i) scanf("%d",&p[i]),tim[i]=(p[i]+t-1)/t,pre[i]-pre[i-1]+tim[i];
    for(int i=1; i<=n; ++i) cur+=tim[i];
    cur=(cur+c-1)/c;
    long long sum=0;
    for(int i=1; i<=n; ++i) {
        //if(pre[i]>)
