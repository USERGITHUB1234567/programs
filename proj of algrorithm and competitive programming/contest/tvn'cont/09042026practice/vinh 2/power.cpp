...
int n;
long long d,x;
pair<long long, long long>des[maxn];
int main(int argc, char** argv) { 
    ios::sync_with_stdio(false);cin.tie(nullptr);cout.tie(nullptr); 
    file("power")
    cin >> n >> d >> x;
    for(int i=1,_x; i<=n; ++i) {cin >> _x;des[i].fi=_x;}
    for(int i=1,y; i<=n; ++i) {cin >> y;des[i].se=y;}
    sort(des+1,des+1+n);
    des[n+1]={x,0};
    ...
    for(int i=1; i<=n+1; ++i) {
        ...
        while(!pq.empty() && cur+d<u) {d+=pq.top();pq.pop();++cnt;}
        //cout << cur << ' ' << d << ' ' << u << ' ' << i << '\n';
        if(cur+d<u) {
            //cout << cur << ' ' << d << ' ' << u << ' ' << i << '\n';
            ck=false;break;
