...
const int maxn=5000006,mod=1000000007,maxb=320; 
namespace utilities{ 
...
namespace soup1{
    ...
    void solve() {
        ...
        for(int i=1; i<=k; ++i) {
            // string t=s;
            for(int j=1; j<=n; ++j) t[rev[j]-1]=s[j-1];
            //cout << t << '\n';
            swap(s,t);
            // s=t;
...
namespace soupfull{
    int rev[maxn],up[maxn][49];
    inline int jump(int i) {
        for(long long dif=k; dif; dif&=(dif-1)) {
            int j=__builtin_ctz(dif);
            i=up[i][j];
        }
        return i;
    }
